...
const ORBS = [
  { rx: 0.5,  ry: -0.12, r: 340, color: 'rgba(99,102,241,',  a: 0.12, phase: 0,             speed: 0.00065 },
  { rx: 0.88, ry:  0.35, r: 230, color: 'rgba(6,182,212,',   a: 0.09, phase: Math.PI / 2,   speed: 0.00085 },
  { rx: 0.1,  ry:  0.70, r: 190, color: 'rgba(99,102,241,',  a: 0.07, phase: Math.PI,       speed: 0.00105 },
  { rx: 0.75, ry:  0.85, r: 160, color: 'rgba(6,182,212,',   a: 0.06, phase: Math.PI * 1.5, speed: 0.00070 },
];
...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    function frame(t: number) {
      ...
      ORBS.forEach(o => {
        ...
        const cx = W * o.rx, cy = H * o.ry + drift;
      ...
      for (let x = 0; x < W; x += sp) {
        for (let y = 0; y < H; y += sp) {
          ...
          const al = Math.max(0, (1 - d / (Math.sqrt(W*W + H*H) * 0.42)) * 0.09);
  ...
  function typeText(str: string) {
    ...
    setIsTyping(true); setPrompt('');
    let i = 0;
    typeTimer.current = setInterval(() => {
      i++;
      setPrompt(str.slice(0, i));
      if (i >= str.length) { clearInterval(typeTimer.current!); typeTimer.current = null; setIsTyping(false); }
    }, 26);
  }

  function handleSuggestion(label: string) { typeText(`A ${label.toLowerCase()} page with modern design`); }
  function handleWand() { typeText(WAND_PHRASES[wandIdx.current++ % WAND_PHRASES.length]); }
    ...
    <div className={`hero-page${mounted ? ' hero-page--mounted' : ''}`}>
      ...
      <section className="hero-section">
        ...
        <h1 className="hero-h1">
          What should we{' '}
          <span className="hero-h1-grad">design?</span>
        </h1>
        ...
        <div className={`hero-card${hasPrompt ? ' hero-card--active' : ''}`}>
          <div className="hero-card-shell">
            ...
            <Textarea
              ...
              value={prompt}
              onChange={(e) => { if (!isTyping) setPrompt(e.target.value.slice(0, MAX_CHARS)); }}
            ...
            <Button
              className={`hero-gen-btn hero-gen-btn--corner${genPulse ? ' hero-gen-btn--pulse' : ''}`}
              type="button"
          ...
          <div className="hero-card-footer">
            <div className="hero-icon-row">
              ...
              <button className="hero-icon-btn hero-icon-btn--wand" title="AI enhance" type="button" onClick={handleWand}>
                <Wand2 size={15} />
              </button>
            ...
            {hasPrompt && (
              <span className="hero-char">{prompt.length}/{MAX_CHARS}</span>
            )}
          </div>
        </div>
        ...
        <div className="hero-chips">
          ...
          {SUGGESTIONS.map(({ label, icon: Icon }) => (
            <button key={label} className="hero-chip2" type="button" onClick={() => handleSuggestion(label)}>
              <Icon size={12} />
              {label}
            </button>
          ))}
